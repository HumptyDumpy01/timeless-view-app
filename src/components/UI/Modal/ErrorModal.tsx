...
import ContactMeBadge from '../ContactMeBadge.tsx';
import { OLENAS_INSTAGRAM } from '../../../../utils/variables.ts';

function ErrorModal({ onClose, errorMessage }: ErrorModalType) {
  return (
    <>
      ...
      transition-all duration-200 hover:rotate-180 hover:scale-110`}>
        ...
        <Heading heading={(
          <div className={`font-raleway font-semibold sm:text-7xl text-5xl max-w-screen-md mb-16`}>
            Oops! Something <HighlightText highlightColor={`red`} text={`went wrong here!`} />
          </div>
        )} />
        <div className={`flex flex-col gap-5 `}>
          <Paragraph customClasses={`max-w-md`} text={(
            <>
              {errorMessage}
            </>
          )} />
